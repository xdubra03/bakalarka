...
class Etree(Tree):
    ...
    _matrix = BlosumMatrix('./blosum62.txt')
				...
				for child in children:
					...
					for parentItem in node._names:
						...
						for childItem in child._names:
							...
							probability = self._probability_matrix.find_pair(seq1,seq2)
...
class MutationRecord():
    ...
    def run_blast(self):
    	subprocess.call(['./blastp','-query','%s.fasta'%self.pdb_id,'-db','nr','-outfmt','5','-out','%s.xml'%self.pdb_id,'-max_target_seqs','250','-remote'])
    ...
    def conservation(self,newick_file,clustal_file):
        t = Etree(newick_file)
    ...
    def create_record(self):
        ...
        x = self.wild_type
        y = self.mutant
        conservation = self.conservation(self.pdb_id+"_newick",self.pdb_id+"clustal.fasta")
        ...
        record.write('%s,%s,%s,%s,%s,%s,%s' %(conservation_code,f_polarity,f_charge,f_hydro_index,struc_id,asa_val,size))
        record.close()
